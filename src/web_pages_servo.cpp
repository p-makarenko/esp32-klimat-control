...
void handleServoPage() {
    ...
    html += "      let mode = data.split(':')[1];";
    html += "      let isOn = mode === 'ON';";
    html += "      // Оновлюємо кнопку калібрування";
    html += "      document.getElementById('calibBtn').innerText = isOn ? '🔓 ВИЙТИ З КАЛІБРУВАННЯ' : '🔒 УВІЙТИ В КАЛІБРУВАННЯ';";
    html += "      // Оновлюємо статус режиму";
    html += "      let modeSpan = document.getElementById('mode');";
    html += "      if(modeSpan) {";
    html += "        modeSpan.innerText = isOn ? 'КАЛІБРУВАННЯ' : 'НОРМАЛЬНИЙ';";
    html += "        modeSpan.style.color = isOn ? '#FF9800' : '#2196F3';";
    html += "      }";
    html += "    } else {";
    ...
    html += "  let calibBtn = document.getElementById('calibBtn');";
    html += "  calibBtn.disabled = true;";
    html += "  sendCommand('calibration:toggle');";
    html += "  setTimeout(() => calibBtn.disabled = false, 500);";
    html += "}";
    ...
    html += "    let testBtn = event.target;";
    html += "    testBtn.disabled = true;";
    html += "    sendCommand('test');";
    html += "    setTimeout(() => testBtn.disabled = false, 3000);";
    html += "  }";
    html += "}";
    html += "</script>";
