...
void handleServoPage() {
    ...
    html += "    } else if(data === 'CALIB:ON' || data === 'CALIB:OFF') {";
    html += "      let isOn = data === 'CALIB:ON';";
    html += "      console.log('🔧 Калібрування:', isOn ? 'УВІМКНЕНО' : 'ВИМКНЕНО');";
    html += "      let calibBtn = document.getElementById('calibBtn');";
    html += "      if(calibBtn) calibBtn.innerText = isOn ? '🔓 ВИЙТИ З КАЛІБРУВАННЯ' : '🔒 УВІЙТИ В КАЛІБРУВАННЯ';";
    html += "      let modeSpan = document.getElementById('mode');";
    html += "      if(modeSpan) { modeSpan.innerText = isOn ? 'КАЛІБРУВАННЯ' : 'НОРМАЛЬНИЙ'; modeSpan.style.color = isOn ? '#FF9800' : '#2196F3'; }";
    html += "    } else {";
