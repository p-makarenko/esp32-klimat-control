...
void handleServoPage() {
    ...
    html += "      if(calibBtn) {";
    html += "        calibBtn.innerText = isOn ? '🔓 ВИЙТИ З КАЛІБРУВАННЯ' : '🔒 УВІЙТИ В КАЛІБРУВАННЯ';";
    html += "        calibBtn.disabled = false;  // Розблоковуємо кнопку";
    html += "      }";
    ...
    html += "      }";
    html += "      console.log('✅ Режим калібрування: ' + (isOn ? 'ВІД' : 'ВИ') + 'МКНЕНО');";
    html += "    } else {";
